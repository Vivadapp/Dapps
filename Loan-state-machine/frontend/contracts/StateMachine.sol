...
contract StateMachine {
    ...
    function _transitionTo(State to) internal {
      require(to != State.PENDING, 'cannot go back to pending'); //can not go to initial state
      require(to != state, 'cannot transition to same state'); //can not go to current state
      if(to == State.ACTIVE) {
