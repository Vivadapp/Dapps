...
contract Ebay {
    ...
    struct Auction {
        ...
        string name;
        string description;
        uint min; //minimum price of the auction
        uint end; //end date of the auction
        uint bestOfferId;
        uint[] offerIds; //array of all the offers that were made for this auction
    }
    ...
    function createOffer(uint _auctionId) external payable auctionExists(_auctionId) {
        Auction storage auction = auctions[_auctionId]; //auction on which we want to make an offer
        Offer storage bestOffer = offers[auction.bestOfferId]; //pointer to the best offer
